  ...
  const AddToCart = async (userId, productId) => {
    try {
      ...
      if (existingOrderIndex !== -1) {
        ...
        const productInOrderIndex = existingOrder.products.findIndex(
    ...
    try {
      ...
      if (order) {
        ...
        const updatedTotalPrice = updatedProducts.reduce((total, product) => {
          ...
          return (
            total + (productData ? productData.price * product.quantity : 0)
          );
        }, 0);
        ...
        setOrders(
          orders.map((order) => (order.id === orderId ? newOrder : order))
        );
      }
    } catch (error) {
