...
function Store() {
  ...
  useEffect(() => {
    fetch('http://localhost:5000/stores')  // Gọi API thay vì lấy file JSON cục bộ
      .then((response) => {
        if (!response.ok) {
      ...
      .then((data) => {
        setStores(data);  // Dữ liệu từ API trả về
              ...
              <div className="map-container">
                ...
                >
                  {filteredStores.map((store) => (
                   ...
                   title={store.name}
                   icon={{
                     url: "/locale.png", // Đường dẫn ảnh tùy chỉnh
                     scaledSize: new window.google.maps.Size(30, 40), // Kích thước ảnh
                   }}
                 />
